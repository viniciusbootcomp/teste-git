...
export default function AdminPedidoDetalhePage() {
  ...
  const carregarPedido = useCallback(async () => {
    ...
    if (pedidoError) {
      setMensagem(`Erro ao carregar pedido: ${pedidoError.message}`);
      setCarregando(false);
      return;
    }
    ...
    if (retiradasError) {
      setMensagem(`Erro ao carregar retiradas: ${retiradasError.message}`);
      setCarregando(false);
      return;
    }
    ...
    setRetiradas(retiradasCarregadas);

    if (retiradasCarregadas.length > 0) {
      const idsRetiradas = retiradasCarregadas.map(
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-8 rounded-xl border border-gray-300 bg-gray-50 p-5">
          ...
          <p className="mt-2 text-gray-600">
            Este pedido possui {retiradas.length}{" "}
            {retiradas.length === 1 ? "retirada." : "retiradas."}{" "}
            Cada unidade opera somente os produtos destinados a ela.
          </p>
        </div>
        ...
        {retiradas.length === 0 ? (
          ...
          <div className="space-y-6">
            {retiradas.map((retirada) => {
              ...
              const produtos = itensRetirada.filter(
                (item) => item.retirada_id === retirada.id
              );
                  ...
                  <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                    <div>
                      ...
                      <h2 className="mt-1 text-2xl font-bold">
                        {unidade?.nome ?? "Unidade O Box Driver"}
                      </h2>
                        ...
                        >
                          <div>
                            <p className="font-semibold">
                              {item.itens_pedido?.nome_produto ?? "Produto"}
                            </p>

                            <p className="text-sm text-gray-500">
                              {item.itens_pedido?.codigo_produto ?? "-"}
                            </p>
                          </div>
