...
export default function AdminPedidosPage() {
  ...
  useEffect(() => {
    const carregar = window.setTimeout(async () => {
      ...
      const { data: pedidosData, error: pedidosError } = await supabase
        .from("pedidos")
        .select(
          "id, numero_pedido, created_at, status_pagamento, total, user_id"
        )
      ...
      const pedidosCarregados = (pedidosData ?? []) as Pedido[];
      ...
      const idsPedidos = pedidosCarregados.map((pedido) => pedido.id);

      const { data: retiradasData, error: retiradasError } = await supabase
        .from("retiradas_pedido")
        .select("id, pedido_id, sequencia, status")
      ...
      setRetiradas((retiradasData ?? []) as RetiradaPedido[]);
      setCarregando(false);
    }, 0);
  ...
  const retiradasPorPedido = useMemo(() => {
    const mapa = new Map<string, RetiradaPedido[]>();

    for (const retirada of retiradas) {
      const lista = mapa.get(retirada.pedido_id) ?? [];
      lista.push(retirada);
      mapa.set(retirada.pedido_id, lista);
    }
  ...
  function obterStatusOperacao(pedidoId: string) {
    const lista = retiradasPorPedido.get(pedidoId) ?? [];

    if (lista.length === 0) {
      return "novo";
    }

    const status = lista.map((retirada) => retirada.status);

    if (status.every((item) => item === "cancelado")) {
      return "cancelado";
    }

    if (status.every((item) => item === "entregue")) {
      return "entregue";
    }

    if (status.some((item) => item === "cliente_no_local")) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {pedidos.length === 0 ? (
          ...
          <div className="space-y-4">
            {pedidos.map((pedido) => {
              const statusOperacao = obterStatusOperacao(pedido.id);
                    ...
                    <div className="flex flex-col gap-3 md:min-w-[360px]">
                      <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                        ...
                        >
                          <span>Operação:</span>{" "}
                          <span className="font-bold">
                            {traduzirOperacao(statusOperacao)}
                          </span>
                        </div>
                      </div>
