...
export async function POST(
  ...
  try {
    ...
    const {
      ...
      await supabaseAdmin.rpc(
        "preparar_pagamento_cartao_credito",
        {
          p_reserva_id:
            reservaId,
        }
    ...
    if (preparacaoError) {
      console.error(
        "Erro ao preparar cartão de crédito:",
        preparacaoError
      );
    ...
    const preparacao =
      preparacaoData as PreparacaoCartao;
    ...
    if (
      ...
      /*
       * =====================================================
       * 9. CRIA ORDER DE CARTÃO DE CRÉDITO
       * =====================================================
       */
  ...
  JSON.stringify(
        ...
        return NextResponse.json(
          {
            ...
            mensagem:
              order.message ??
              "O Mercado Pago recusou o pagamento com cartão.",
      ...
      return NextResponse.json(
        {
          sucesso: false,
          mensagem:
            "A Order retornada não corresponde a um pagamento com cartão de crédito.",
        },
        {
          status: 409,
        }
    ...
    if (
      ...
      console.error(
        "Erro ao salvar pagamento com cartão:",
        atualizarPagamentoError
      );
    ...
    console.error(
      "Erro inesperado ao processar cartão de crédito:",
      error
    );
    ...
    return NextResponse.json(
      {
        sucesso: false,
        mensagem:
          "Erro interno ao processar pagamento com cartão.",
      },
      {
        status: 500,
      }
