-- =========================================================
-- 039 - PERMISSÕES OPERACIONAIS DO PERFIL SEPARAÇÃO
-- O Box Driver - Homologação / Fase 1
--
-- Objetivo:
-- permitir que usuários com tipo_usuario = 'separacao'
-- iniciem a separação e registrem a conferência dos produtos,
-- sem transformá-los em administradores.
--
-- A entrega física continua fora deste perfil.
-- =========================================================


-- =========================================================
-- 1. INICIAR SEPARAÇÃO DE UMA RETIRADA
-- =========================================================
...
begin
  ...
  if not exists (
    ...
    where user_id = v_user_id
      and ativo = true
      and tipo_usuario in ('admin', 'separacao')
  ) then
    raise exception
...
-- =========================================================
-- 2. REGISTRAR LEITURA DE PRODUTO NA RETIRADA
-- =========================================================
...
comment on function public.iniciar_separacao_retirada(uuid)
is
'Permite que perfis admin e separacao iniciem a separação de uma retirada.';

comment on function public.registrar_leitura_separacao_retirada(uuid, text)
is
'Permite que perfis admin e separacao registrem a conferência dos produtos de uma retirada.';
